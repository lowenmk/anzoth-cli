...
GITHUB_REPO = "lowenmk/anzoth-cli"
BINARY_TARGETS = (
...
_SPEC = importlib.util.spec_from_file_location("anzoth_build_npm_package", BUILD_SCRIPT)
...
ANZOTH_PLATFORM_PACKAGES = getattr(_BUILD_MODULE, "PLATFORM_PACKAGES", {})
ANZOTH_PACKAGE_COMPONENT = getattr(_BUILD_MODULE, "PACKAGE_COMPONENT", "anzoth-package")
...
BINARY_COMPONENTS = {
    "anzoth-responses-api-proxy": BinaryComponent(
        artifact_prefix="anzoth-responses-api-proxy",
        dest_dir="anzoth-responses-api-proxy",
        binary_basename="anzoth-responses-api-proxy",
    ),
}
...
def install_from_workflow_artifacts(
    ...
    if ANZOTH_PACKAGE_COMPONENT in components:
        install_anzoth_package_archives(artifacts_dir, vendor_dir, BINARY_TARGETS)
...
def select_target_artifacts(
    ...
    needs_target_artifacts = ANZOTH_PACKAGE_COMPONENT in components or any(
...
def install_anzoth_package_archives(
    artifacts_dir: Path,
    vendor_dir: Path,
    ...
    print("Installing Anzoth package archives for targets: " + ", ".join(targets), flush=True)
    max_workers = min(len(targets), max(1, (os.cpu_count() or 1)))
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(
                install_single_anzoth_package_archive,
                artifacts_dir,
                vendor_dir,
                target,
            ): target
...
def install_single_anzoth_package_archive(
    artifacts_dir: Path,
    vendor_dir: Path,
    target: str,
) -> Path:
    artifact_subdir = artifact_dir_for_target(artifacts_dir, target)
    archive_path = artifact_subdir / f"anzoth-package-{target}.tar.gz"
...
def tarball_name_for_package(package: str, version: str) -> str:
    if package in ANZOTH_PLATFORM_PACKAGES:
        platform = package.removeprefix("anzoth-")
        return f"anzoth-npm-{platform}-{version}.tgz"
