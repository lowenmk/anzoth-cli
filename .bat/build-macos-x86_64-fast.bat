...
set "HOST=mac"
set "DEST=C:\ai\anzoth-cli\releases\macos-x86_64-fast\anzoth"

ssh %HOST% "set -e; cd ~/anzoth-mac-validation; echo FAST DEVELOPMENT BUILD; git fetch origin; git checkout anzoth-rebrand; git pull --ff-only origin anzoth-rebrand; git rev-parse HEAD; cargo build --profile fast-release --manifest-path codex-rs/Cargo.toml -p codex-cli --bin anzoth --target x86_64-apple-darwin; ./codex-rs/target/x86_64-apple-darwin/fast-release/anzoth --version; ls -lh codex-rs/target/x86_64-apple-darwin/fast-release/anzoth; shasum -a 256 codex-rs/target/x86_64-apple-darwin/fast-release/anzoth; file codex-rs/target/x86_64-apple-darwin/fast-release/anzoth" || exit /b 1

if not exist "C:\ai\anzoth-cli\releases\macos-x86_64-fast" mkdir "C:\ai\anzoth-cli\releases\macos-x86_64-fast"
scp %HOST%:~/anzoth-mac-validation/codex-rs/target/x86_64-apple-darwin/fast-release/anzoth "%DEST%" || exit /b 1
