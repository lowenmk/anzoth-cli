...
- Use JavaScript `exec` only for JavaScript Code Mode orchestration.
- For shell execution, use `await tools.exec_command({ cmd: "..." })`.
- The `exec` input must be JavaScript source; do not pass raw shell syntax as the `exec` input. Put the shell command string in the `cmd` field of `tools.exec_command`.
- For file edits, use `tools.apply_patch` with a complete patch when that nested tool is available.
...
bar"
    ...
    fn exec_description_mentions_timeout_helpers() {
        ...
        assert!(description.contains("`await tools.exec_command({ cmd: \"...\" })`"));
        assert!(description.contains("must be JavaScript source"));
        assert!(description.contains("do not pass raw shell syntax as the `exec` input"));
        assert!(
            description
                .contains("Use JavaScript `exec` only for JavaScript Code Mode orchestration")
        );
